...
/**
 * Resolve credentials with AWS-style precedence
 * Priority: 1. Environment variables (all must be set) 2. Profile from config file
 * @param {string} profileName - Name of the profile to use as fallback
 * @returns {Object} Object with credentials and source ('environment', 'profile', or null)
 */
function resolveCredentials(profileName = 'default') {
  // Priority 1: Environment variables (all must be set)
  ...
  if (profile) {
    return { credentials: profile, source: 'profile' };
  }
